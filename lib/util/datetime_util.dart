...
enum DateTimeFormatUtil { BR_DATE }
mixin DateTimeUtil {
  ...
  DateTime strToDate(String value, DateTimeFormatUtil format) {
    if (format == DateTimeFormatUtil.BR_DATE) {
      ...
      return DateTime(
          int.parse(dateStr[2]), int.parse(dateStr[1]), int.parse(dateStr[0]));
    }
  ...
  String getMonth(int month) {
    if (month > 12) {
      month = this.millisecondsToMonth(month);
    }
  ...
  String getMonthYear(int millisecondsSinceEpoch, bool isFullYear) {
    DateTime dateTime =
        DateTime.fromMillisecondsSinceEpoch(millisecondsSinceEpoch);
    String year = (isFullYear)
        ? dateTime.year.toString()
        : dateTime.year.toString().substring(2);
    String month =
        this.months[dateTime.month - 1].substring(0, 3).toUpperCase();
    return '$month/$year';
  }
